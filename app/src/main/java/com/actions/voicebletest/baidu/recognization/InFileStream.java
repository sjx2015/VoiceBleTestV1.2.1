...
public class InFileStream {
    ...
    public static InputStream create8kStream() {
        if (is != null) {
            return new FileAudioInputStream8k(is);
        } else if (filename != null) {
            try {
                return new FileAudioInputStream8k(filename);
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
        } else {
            return new FileAudioInputStream8k(createFileStream());
        }
        return null;
    }
